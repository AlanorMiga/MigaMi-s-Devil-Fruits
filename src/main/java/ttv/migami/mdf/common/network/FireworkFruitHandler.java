        ...
        if (!pLevel.isClientSide()) {
            ...
            switch (move) {
                case 1:
                    ...
                    firework = new CustomFireworkRocketEntity(pLevel, getFireworkStack(false, false, 0, 1), pShooter, pShooter.getX(), pShooter.getEyeY() - 0.15000000596046448, pShooter.getZ(), true);
                    speed = 1.5;
                ...
                case 2:
                    ...
                    if (entityHitResult != null) {
                        firework = new DragFireworkRocketEntity(pLevel, getFireworkStack(false, false, 0, 1), pShooter, entityHitResult.getLocation().x + 0.5, entityHitResult.getLocation().y + 1, entityHitResult.getLocation().z + 0.5);
                    }
                    else {
                        firework = new DragFireworkRocketEntity(pLevel, getFireworkStack(false, false, 0, 1), pShooter, blockPos.getX() + 0.5, blockPos.getY() + 1, blockPos.getZ() + 0.5);
                    }
                    pLevel.addFreshEntity(firework);
                    break;
                ...
                case 5:
                    actionSlowdown(pShooter);
                    firework = new DragFireworkRocketEntity(pLevel, getFireworkStack(false, false, 0, 1), pShooter, pShooter.getX(), pShooter.getEyeY() - 0.15000000596046448, pShooter.getZ(), true);
                    speed = 1.5;
                    firework.lerpMotion(look.x * speed, look.y * speed, look.z * speed);
                    pLevel.addFreshEntity(firework);
                    break;
